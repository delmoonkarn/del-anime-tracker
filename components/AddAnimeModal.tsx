              ...
              <Field label="Platform URL">
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="https://…"
                    className="flex-1 min-w-0 px-3 py-2 rounded-lg bg-zinc-950 border border-zinc-800 focus:border-indigo-500 outline-none text-sm"
                  />
                  <button
                    type="button"
                    disabled={!title.trim()}
                    onClick={() => {
                      const q = encodeURIComponent(`hareshi ${title.trim()}`);
                      window.open(
                    ...
                    title="Search Google for: hareshi {title}"
                    className="shrink-0 inline-flex items-center gap-1 px-2.5 rounded-lg bg-zinc-800 hover:bg-indigo-500 text-zinc-200 hover:text-white text-xs font-medium disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
